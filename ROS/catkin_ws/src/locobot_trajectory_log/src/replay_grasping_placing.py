...
def main():
    ...
    parser.add_argument('path2', type=str, help="input trajectory2 csv file")
    ...
    target_joints2 = []
    with open(args.path2) as csvfile2:

        rows = csv.reader(csvfile2)

        for row in rows:
            target_joints2.append(row)

    target_joints2_re = [target_joints2[-i] for i in range(len(target_joints2))]

    for joint in target_joints2_re[1:-1:args.fps/args.ofps]:
