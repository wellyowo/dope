...
class color_image_repub():
    def __init__(self):
        self.easy_socket = socket.ros_socket('192.168.50.13', 9090)
        self.easy_socket.subscriber('/camera/color/image_raw/compressed', self.sub_callback, 1)
        self.image_pub = rospy.Publisher('/camera/color/image_raw/compressed', CompressedImage, queue_size = 1)
...
class depth_image_repub():
    def __init__(self):
        self.easy_socket = socket.ros_socket('192.168.50.13', 9090)
        self.easy_socket.subscriber('/camera/aligned_depth_to_color/image_raw/compressedDepth', self.sub_callback, 1)
        self.image_pub = rospy.Publisher('/camera/aligned_depth_to_color/image_raw/compressedDepth', CompressedImage, queue_size = 1)
        self.compressed_image = CompressedImage()
...
class butter_pose_repub():
    def __init__(self):
        self.easy_socket = socket.ros_socket('192.168.50.13', 9090)
        self.talker2 = self.easy_socket.publisher('/dope/pose_butter', 'geometry_msgs/PoseStamped')
        self.pose_sub2 = rospy.Subscriber('/dope/pose_Butter', PoseStamped, self.sub_callback)
        self.data2 = {'header':{'seq':0, 'stamp':{'secs':0, 'nsecs':0},'frame_id':''}, 'pose':{'position':{'x':0.0, 'y':0.0, 'z':0.0}, 'orientation':{'x':0.0, 'y':0.0, 'z':0.0, 'w':0.0}}}
    
    
    
    def sub_callback(self, msg):
        self.data2['header']['seq'] = msg.header.seq
        self.data2['header']['stamp']['secs'] = msg.header.stamp.secs
        self.data2['header']['stamp']['nsecs'] = msg.header.stamp.nsecs
        self.data2['header']['frame_id'] = msg.header.frame_id
        self.data2['pose']['position']['x'] = msg.pose.position.x
        self.data2['pose']['position']['y'] = msg.pose.position.y
        self.data2['pose']['position']['z'] = msg.pose.position.z
        self.data2['pose']['orientation']['x'] = msg.pose.orientation.x
        self.data2['pose']['orientation']['y'] = msg.pose.orientation.y
        self.data2['pose']['orientation']['z'] = msg.pose.orientation.z
        self.data2['pose']['orientation']['w'] = msg.pose.orientation.w
        self.easy_socket.pub(self.talker2, self.data2)
...
if __name__=='__main__':
    ...
    pose = sugar_pose_repub()
    pose = butter_pose_repub()
